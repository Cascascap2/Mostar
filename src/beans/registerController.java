	...
	public boolean validatePassword (){
		...
		if(password != null && password.length() >= 8) {
			if(this.repeatPassword.equals(password)) {
				this.setMessages("Contrase�a valida.");
				context.addMessage(null, new FacesMessage("Info", this.Messages));
				return true;
			}
			else {
				this.setMessages("Las Contrase�as deben ser iguales.");
				context.addMessage(null, new FacesMessage("Info", this.Messages));
				return false;
			}
			
		}else{
			this.setMessages("Contrase�a invalida debe contener al menos 8 caracteres.");
			context.addMessage(null, new FacesMessage("Info", this.Messages));
			return false;
		}
